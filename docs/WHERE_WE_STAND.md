...
- Phase 2 is an active but incomplete package-backed workflow; the new recording-first front half is implemented, but it is not a release claim or a replacement for the protected Phase 1 lane until real-media round-trip verification is complete.
...
- Full answer-boundary QA and real-media verification of multipart answer composition.
- Cancellation/retry UX for long imports and publications.
- Incremental manifest editing without generating a new publication.
- Timeline UI.
- Deeper Phase 2 scrubbing/trimming and clip-factory tooling.
...
- The current UI is intentionally conservative and keeps the new workflow in native SwiftUI/AppKit controls.
...
- The current checkout has a passing packaged app build, a launch smoke check, and a full SwiftPM suite with 36 tests passing and one intentional renderer-smoke skip. A real birthday-project source → candidate → publication → Phase 1 render round trip and owner playback/perceptual review remain open before this becomes a release or media-quality claim.
